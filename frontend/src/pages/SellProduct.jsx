...
export default function SellProduct() {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (res.ok) {
        alert(editingId ? 'Product updated!' : 'Product listed!');
        setFormData({
          product_name: '',
          category: categories[0],
          quantity: '',
  ...
  return (
    <div className="sell-product">
      <h2>{editingId ? 'Edit Product' : 'Sell Your Product'}</h2>
      <form onSubmit={handleSubmit} className="product-form">
        ...
        <button type="submit" disabled={uploading} className="submit-btn">
          {uploading ? 'Uploading...' : editingId ? 'Update Product' : 'List Product'}
        </button>
      </form>

      <h3 style={{ marginTop: '3rem' }}>Your Listed Products</h3>
        ...
        {userProducts.map(product => (
          <motion.div key={product.id} className="product-item" whileHover={{ y: -5 }}>
            ...
            <div className="product-info">
              ...
              <p>{product.category} – ₹{product.price} / {product.quantity}</p>
